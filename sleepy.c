...
sleepy_write(struct file *filp, const char __user *buf, size_t count, 
	     loff_t *f_pos)
{
  int *sleepySeconds = kmalloc(4, GFP_KERNEL);
  int sleepInJiffies;
  int timeLeft;
  ...
  /* YOUR CODE HERE */
  if(count != 4) {
    printk("Must write exactly 4 bytes");
    return -EINVAL;
  }
  copy_from_user(sleepySeconds, buf, count);

  printk("Number of seconds to sleep %d\n", *sleepySeconds);

  if(*sleepySeconds < 0) return 0;
  sleepInJiffies = *sleepySeconds * HZ;
  /* END YOUR CODE */	
  mutex_unlock(&dev->sleepy_mutex);

  timeLeft = wait_event_interruptible_timeout(dev->wq, dev->flag, sleepInJiffies);

  mutex_lock(&dev->sleepy_mutex);
  dev->flag = 0;
  mutex_unlock(&dev->sleepy_mutex);
  retval = timeLeft/HZ;
  printk("Time left to sleep %d\n", retval);
  return retval;
}
			...
			struct class *class)
{
  ...
  // Initialize the wait queue and flag variable.
